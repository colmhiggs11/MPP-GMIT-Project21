...
struct Customer custOrder(char *filename)

{
	...
	while ((read = getline(&line, &len, fp)) !=-1) {
		...
		struct Product prod_wanted = {name,0.0};
		struct ProductStock SL_Item = {prod_wanted,quantity};
		customer.shoppingList[customer.index++] = SL_Item;
...
{	
	...
	struct Customer customer = custOrder("../Customer_b.csv");
